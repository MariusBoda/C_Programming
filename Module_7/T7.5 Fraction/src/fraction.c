...
{
    unsigned int a_numerator = a->numerator * b->denominator;
    unsigned int b_numerator = b->numerator * a->denominator;

    if (a_numerator < b_numerator)
        return -1;
    else if (a_numerator > b_numerator)
        return 1;
    else
        return 0;
}
...
Fraction *addFraction(const Fraction *a, const Fraction *b)
{
    ...
    if (res != NULL) {
        unsigned int common_denom = a->denominator * b->denominator;
        unsigned int a_numerator = a->numerator * b->denominator;
        unsigned int b_numerator = b->numerator * a->denominator;

        res->numerator = a_numerator + b_numerator;
        res->denominator = common_denom;
    }
    return res;
}
